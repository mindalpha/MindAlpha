...
spark = ma.spark.get_session(batch_size=batch_size,
...
with spark:
    module = ma.nn.WideAndDeepModule(
    ...
    train_dataset = ma.input.read_s3_csv(spark, train_dataset_path, delimiter='\t')
    model = estimator.fit(train_dataset)

    test_dataset_path = S3_ROOT_DIR + 'demo/data/test/day_0_0.001_test.csv'
    test_dataset = ma.input.read_s3_csv(spark, test_dataset_path, delimiter='\t')
