...
template<typename T>
std::shared_ptr<T> extract_shared_pyobject(pybind11::object obj)
{
...
template<typename T>
inline pybind11::array make_numpy_array(SmartArray<T> data)
{
...
template<typename T>
inline pybind11::array to_numpy_array(std::vector<T> data)
{
...
template<typename T>
pybind11::tuple make_python_tuple(const std::vector<T>& vec)
{
...
template<typename T>
pybind11::list make_python_list(const std::vector<T>& vec)
{
    pybind11::list result(vec.size());
    for (size_t i = 0; i < vec.size(); i++)
        result[i] = vec.at(i);
...
template<typename T>
std::vector<T> make_cpp_vector(pybind11::object obj)
{
    std::vector<T> result;
