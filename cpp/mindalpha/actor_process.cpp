...
#include <mindalpha/network_utils.h>
...
ActorProcess::ActorProcess(std::shared_ptr<ActorConfig> config)
    ...
    , transport_(MessageTransport::Create(config_))
...
void ActorProcess::Barrier(int group)
{
    manager_->Barrier(group, *this);
}
...
void ActorProcess::Receiving()
{
    spdlog::info("ActorProcess::Receiving: {} {}",
                 NodeRoleToString(config_->GetThisNodeInfo().GetRole()),
                 GetThreadIdentifier());
...
#define MINDALPHA_NODE_CONTROL_COMMAND_DEF(n)     \
...
void ActorProcess::UpdateLocalId(const Message& msg)
{
    ...
    if (msg.GetMessageMeta().GetSender() == -1)
    {
        ...
        if (control.GetNodes().size() != 1)
        {
            std::string serr;
            serr.append(config_->GetThisNodeInfo().ToShortString());
            serr.append(": Message from unknown sender must contain exactly one node info, but found ");
...
void ActorProcess::CoordinatorHandleAddNode(const Message& msg)
{
    ...
    if (nodes_.GetNodeControl().GetNodes().size() == numNodes)
    {
        std::sort(nodes_.GetNodeControl().GetNodes().begin(),
                  nodes_.GetNodeControl().GetNodes().end(),
                  [](const NodeInfo& x, const NodeInfo& y) {
                      return (x.GetHostName().compare(y.GetHostName()) |
                              (x.GetPort() < y.GetPort())) > 0;
                  });
        for (NodeInfo& node: nodes_.GetNodeControl().GetNodes())
        {
            const int id = node.GetRole() == NodeRole::Server ?
                           ServerRankToNodeId(num_servers_) :
            ...
            if (!connected_nodes_.count(addr))
            {
                if (node.GetNodeId() != -1)
                {
                    std::string serr;
                    serr.append(config_->GetThisNodeInfo().ToShortString());
                    serr.append(": Node id is expected to be unsigned, but it has been assigned to ");
