...
NodeManager::NodeManager(std::shared_ptr<ActorConfig> config)
...
void NodeManager::InitNodeIds()
{
    ...
    for (int i = 0; i < config_->GetWorkerCount(); i++)
    {
        ...
        for (int g: { id,
                      WorkerGroup,
                      WorkerGroup | ServerGroup,
                      WorkerGroup | CoordinatorGroup,
    ...
    for (int i = 0; i < config_->GetServerCount(); i++)
    {
        ...
        for (int g: { id,
                      ServerGroup,
                      ServerGroup | WorkerGroup,
                      ServerGroup | CoordinatorGroup,
                      ServerGroup | CoordinatorGroup | WorkerGroup })
            node_ids_[g].push_back(id);
    }
    for (int g: { CoordinatorNodeId,
                  CoordinatorGroup,
                  CoordinatorGroup | ServerGroup,
                  CoordinatorGroup | WorkerGroup,
                  CoordinatorGroup | ServerGroup | WorkerGroup })
...
std::vector<int> NodeManager::GetDeadNodes(int timeout)
{
    ...
    for (int id: nodeIds)
    {
        ...
        if (it == heartbeats_.end() ||
            it->second + timeout < now && start_time_ + timeout < now)
            dead.push_back(id);
    }
    return dead;
}
