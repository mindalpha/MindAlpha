...
#include <iostream>
#include <stdexcept>
#include <mindalpha/zeromq_transport.h>
...
void ZeroMQTransport::Connect(const NodeInfo& node)
{
    ...
    if (node.GetRole() == thisNode.GetRole() &&
        node.GetNodeId() != thisNode.GetNodeId())
        return;
...
int64_t ZeroMQTransport::SendMessage(const Message& msg)
{
    ...
    zmq_msg_t meta;
    zmq_msg_init_data(&meta, metaPtr->data(), metaSize, [](void* ptr, void* hint) {
    ...
    for (;;)
    {
        ...
        zmq_msg_close(&meta);
        std::string serr = fmt::format("{}: Fail to send message meta to node {}, errno: {} {}.\n\n{}",
                                       thisNode.ToShortString(), NodeIdToString(nodeId),
                                       errno, zmq_strerror(errno), GetStackTrace());
        spdlog::error(serr);
        throw std::runtime_error(serr);
    }
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        zmq_msg_t data;
        zmq_msg_init_data(&data, dataPtr->data(), dataSize, [](void* ptr, void* hint) {
        ...
        for (;;)
        {
            ...
            // but call it again is not harmful.
            zmq_msg_close(&data);
            std::string serr = fmt::format("{}: Fail to send message slice to node {}, errno: {} {}. {}/{}\n\n{}",
                                           thisNode.ToShortString(), NodeIdToString(nodeId),
                                           errno, zmq_strerror(errno), i, n, GetStackTrace());
            spdlog::error(serr);
            throw std::runtime_error(serr);
        }
        bytes += dataSize;
...
std::string ZeroMQTransport::FormatActorAddress(const NodeInfo& node, int port, bool forServer) const
{
